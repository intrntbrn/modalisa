...
local function stop(self, stop_key, stop_mods)
	...
	if self.stop_callback then
		self.stop_callback(self.current_instance, stop_key, stop_mods, self.sequence)
	end
...
function keygrab.stop(g)
	...
	-- first keygrabber.
	g = g or keygrab.current_instance and keygrab.current_instance.grabber or grabbers[#grabbers]
...
function keygrabber:add_keybinding(key, keycode, callback, description)
	...
	if mods then
		gdebug.deprecate(
			":add_keybinding now takes `awful.key` objects instead" .. " of multiple parameters",
			{ deprecated_in = 5 }
		)
	...
	elseif keycode or callback or description then
		gdebug.deprecate(
			":add_keybinding only accept a single parameter. All " .. "other were ignored.",
			{ deprecated_in = 4 }
		)
	end
...
function keygrabber:set_root_keybindings(keys)
	...
	for _, key in ipairs(keys) do
		if key._is_awful_key then
			...
			gdebug.deprecate(
				":set_root_keybindings now takes `awful.key` " .. " objects instead of tables",
				{ deprecated_in = 5 }
			)
...
function keygrab.run_with_keybindings(args)
	...
	for _, v in ipairs(args.keybindings or {}) do
		...
		elseif #v >= 3 and #v <= 4 then
			gdebug.deprecate(
				"keybindings now contains `awful.key` objects" .. "instead of multiple tables",
				{ deprecated_in = 5 }
			)
			...
			if type(callback) == "function" then
				...
				gdebug.print_warning("The hook's 3rd parameter has to be a function. " .. gdebug.dump_return(v or {}))
			end
		else
			gdebug.print_warning("The keybindings should be awful.key objects" .. gdebug.dump_return(v or {}))
		end
	end
